...
with open(product_file_path) as f:
        ...
        for row in reader:
            ...
            if count>2:
                ...
                price = float(row[15].replace(",",""))
                if row[16]!="":
                    price_discounted = float(row[16].replace(",",""))
                else:
                    price_discounted = 0
                
                curr_prod,created = Product.objects.get_or_create(store=current_store, sku=row[0], sex=row[1], category=row[2],\
                        name=row[3], brand=row[4],color=row[5],material=row[6],made_to_order=row[14],\
